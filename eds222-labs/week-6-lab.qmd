...
1.  What do you think each row represents?\
    I think each row represents one sample of water measured.
...
```{r}
drinking_water$violation <- as.numeric(drinking_water$health > 0)
drinking_water
```
...
drinking_water_binned <- drinking_water %>% 
  mutate(pcthisp2 = round(pcthisp * 5) / 5) %>% 
  group_by(pcthisp) %>% 
...
logit_p <- beta0 + beta1 * c(0,50,100)
p <- inv_logit(logit_p)
p
...
```{r}
logit_p2 <- beta0 + beta1 * c(20,55,90)
p2 <- inv_logit(logit_p2)
p2

logit_p3 <- beta0 + beta1 * c(10,45,80)
p3 <- inv_logit(logit_p3)
p3
...
4.  How would you interpret the coefficients? What do the slope and intercept mean in this context? Where is the relationship linear, and where is it non-linear?
    -   beta0 is the probability of drinking violation when pcthisp is 0. Beta1 is the change in probability with each unit in change in pcthisp. Its nonlinear?

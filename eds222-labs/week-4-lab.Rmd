...
# B0 = 35.7922, B1 = -3.7684, B2 = 0.3445, B3 = 0.3052

summary(lm(hwy ~ displ + year +displ : year, mpg))

```

2.  What does the intercept tell you, in words?

    -   The intercept tells me when the year is 1999 and the displ is 0 then the hwy is 35.7922

3.  What does the coefficient on `displ` tell you, in words?

    -   B1 tells me the change in displ for unit change in year (to 2008)

4.  What does the coefficient estimate on the 2008 vintage indicator variable (`year2008`) tell you, in words?

    -   The coefficient estimate on 2008 tells me that when the year is 2008 its the change in hwy when displ is zero from the year 1999

5.  What does the coefficient on `displ:year2008` tell you, in words?[^1]

    -   b3 is the change in the change of hwy when the year is 1999 per unit change of displ.
...
ggplot(mpg, aes(displ, hwy, color = year)) +
  geom_point() +
  geom_abline(intercept = 35.7922, slope = -3.7684, color = "coral") +
  geom_abline(intercept = 35.7922 + 0.3445,
              slope = -3.7684 + 0.3052,
              color = "lightblue")
```

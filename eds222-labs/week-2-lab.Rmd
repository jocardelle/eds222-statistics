...
```{r}
pie_crab <- pie_crab
size_lat <- ggplot(data = pie_crab, aes(x = latitude, y = size)) +
  geom_point() +
  labs(title = "Crab Size vs Latitude") +
  theme(aspect.ratio = 1)


temp_lat <- ggplot(data = pie_crab, aes(x = latitude, y = water_temp)) +
  geom_point() +
  labs(title = "Water Temperature vs Latitude")
...
(i) the size of crab and latitude: 0.45
(ii) water temperature and latitude: -0.85
...
```{r}
lm(size ~ lat, pie_crab)

summary(lm(size ~ lat))

# B0 represents crab size at latitude zero. b1 represents the change in crab size with each 1 degree of increase in latitude.

# Our b0 is no directly interpretable because you cant have a negative size
```
...
ggplot(data = pie_crab, aes(x = latitude, y = water_temp)) +
  geom_point() +
  labs(title = "Water Temperature vs Latitude") +
  theme(aspect.ratio = 1) +
  geom_smooth(method = lm, se = FALSE)

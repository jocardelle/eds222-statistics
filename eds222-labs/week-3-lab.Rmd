...
```{r}
summary(lm(formula = size ~ latitude, data = pie_crab))

summary(lm(formula = water_temp ~ latitude, data = pie_crab))

# The water_temp vs latitude regression model fits the data better. The R^2 for size vs latitude is 0.348 and the R^2 for water_temp vs latitude is 0.916. This is what I expected because the second scatter plot has a stronger linear relationship.
```
...
```{r}
# Find SSE for size lat
e_size <- resid(lm(formula = size ~ latitude, data = pie_crab))
SSE_size <- sum((e_size^2))

# Find SSR for size lat
y_bar_size <- mean(pie_crab$size)
SST_size <- sum((pie_crab$size - y_bar_size)^2)
r_squared_size <- (1-(SSE_size/SST_size))

# Find SSE for temp lat
e_water <- resid(lm(formula = water_temp ~ latitude, data = pie_crab))
SSE_water <- sum((e_water^2))

# Find SSR for temp lat
y_bar_water <- mean(pie_crab$water_temp)
SST_water <- sum((pie_crab$water_temp - y_bar_water)^2)
r_squared_water <- (1-(SSE_water/SST_water))
...
# The intercept tells us the hwy when the year is 1999 and the 2008 vintage indicator tells us how much hwy increases by 2008
```

4.  Does your model suggest anything about whether fuel efficiency has evolved over time? Why or why not?
    -   It has increased by about 2% so its very small.
...
```{r}
ggplot(mpg, aes(displ,hwy,col = year_fct)) +
  geom_point()
```
